...
def get_adults():
    response = list(database.db.Badges.find({"$gte": 21}))
...
def get_name_and_age():
    response = list(database.db.Badges.find({'age': {"$gte": 20}}, {'name': 1, 'age':1}))
...
@app.route('/all/kids')
def get_kids():
    response = list(database.db.Badges.find({"$gte": 1}))

    for document in response:
        documnet["_id"] = str(document['_id'])
